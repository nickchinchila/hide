...
import HIDE_parallel_config as hpcf
...
percentage = config.getint('RunHide', 'percentage_consider_core_unavailable')
parallel = config.getboolean('RunHide', 'parallel')
total_cores = mp.cpu_count()
used_cores = np.sum(np.array(ps.cpu_percent(percpu=True, interval=0.1)) > percentage)  
idle_cores = total_cores - used_cores

if parallel:
	print(f"Running {final_horn - initial_horn} horns in parallel")

	args = [(i, working_path, destination_path, dfile_short) 
			for i in range(initial_horn, final_horn)]
	
	with mp.Pool(processes=idle_cores, initializer=hpcf.init_worker) as pool:
		x = pool.starmap(hpcf.run_horn, args)
else:
	for i in range(initial_horn, final_horn):
		print(f"\nExecuting horn {i}\n")
