		...
		for horn in range (horns_i,horns_f):
			...
			for hour in range(24):
				...
				if hour == 23 and last_hour_extra:
					current_n -= 1   
				max_index = min(current_n, len(temp_maps))
		
				for i in range(max_index):
				
					# instant_us = hour * us_per_hour + i * size_interval_us
					# instant = instant_us / 10**6
...
def process_channel(i, seed0, path):
	"""
	Função para processar um canal individual
	"""
	naive_hitmap(path, "2020-03-01", "2020-03-01", step, horns_i=0, horns_f=140, nside=256,channel=i, title=f"signal+noise_SEED{seed0}_ch{i}_nch30_1d",output_path=path + 'maps/')
...
if __name__ == "__main__":
	...
	f_bin = np.arange(30)
	step = config.getfloat('ScanningStrategy', 'strategy_step_size')

	percentage = config.getint('Hitmap', 'percentage_consider_core_unavailable')
	...
	# print("idle_cores", idle_cores)
	
	with mp.Pool(processes=idle_cores) as channel_pool:
			args = [(i, seed0, path) for i in f_bin]
			channel_pool.starmap(process_channel, args)
